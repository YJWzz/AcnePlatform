# -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
#  flask + YOLO-ONNX 痘痘分析 API
#  (c) 2025 — 精簡至必要 import，移除 TensorFlow
# -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
import json
from flask import Flask, request, jsonify, send_from_directory
import cv2, numpy as np, os, pymysql
import requests
...
DB_CONFIG = dict(host='localhost', user='root', password='MySQL',
                 database='acne', charset='utf8mb4')

# ---------- 載入 YOLO 模型 ----------
try:
    MODEL_PATH = os.path.join('detect', 'best.onnx')     #  路徑是字串
